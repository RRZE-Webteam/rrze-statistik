...
jQuery(function ($) {
  var selector = ''; // the Configure link click event
  ...
  $('body').on('submit', '#rrze_statistik_settings', function () {
    ...
    $.ajax({
      url: ajaxurl,
      type: 'POST',
      data: $(this).serialize() + "&selector=".concat(selector),
      // all form fields
      beforeSend: function beforeSend(xhr) {
...
console.log(RRZESTATISTIKTRANSFER.linechartDataset); //data is passed from Transfer.php
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  if (RRZESTATISTIKTRANSFER.linechartDataset === "undefined") {
    ...
    chartTypes.forEach(function (datatype) {
      ...
      var outputThirdYear = filterData(RRZESTATISTIKTRANSFER.linechartDataset, thirdYear);
      var outputSecondYear = filterData(RRZESTATISTIKTRANSFER.linechartDataset, secondYear);
      var outputFirstYear = filterData(RRZESTATISTIKTRANSFER.linechartDataset, firstYear); //Create an empty dataset array
      ...
      var chart = Highcharts.chart(datatype, {
        ...
        tooltip: {
          valueSuffix: " ".concat(RRZESTATISTIKTRANSFER.languagePackage[datatype].ordinate_desc)
        },
        plotOptions: {
          series: {
            cursor: "pointer"
